...
export default function MalfunctionsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const forcedQuery = `${input} (context: malfunction diagnosis symptoms possible causes actions)`;
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-2rem)] max-w-4xl mx-auto p-4 md:p-6">
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-zinc-800/80">
        <div>
          <h1 className="text-xl font-bold text-emerald-400 flex items-center gap-2">
            <Stethoscope className="h-5 w-5" /> Malfunction Diagnostics
          </h1>
          <p className="text-xs text-zinc-500 mt-1">Symptom analysis and recommended actions</p>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto mb-6 pr-2 space-y-6">
        {!surface && !queryApi.loading && (
          <div className="flex flex-col items-center justify-center h-full text-zinc-500 space-y-4">
            <Stethoscope className="h-12 w-12 text-zinc-700/50" />
            <p className="text-sm">Describe a symptom to get a diagnostic card.</p>
          </div>
        )}
        {queryApi.loading && (
          <div className="flex flex-col items-center justify-center py-12 space-y-4">
            <Loader2 className="h-8 w-8 text-emerald-500 animate-spin" />
            <p className="text-sm text-zinc-400 animate-pulse">Running diagnostics...</p>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className="relative mt-auto shrink-0">
        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="E.g., The engine light is on and car is vibrating" className="w-full bg-zinc-900 border border-zinc-700/80 text-white rounded-xl pl-4 pr-12 py-4 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 shadow-lg" disabled={queryApi.loading} />
